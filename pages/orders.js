...
const Orders = () => {
  ...
  return (
    <div className='relative top-24 w-full' style={{margin:'0px',padding:'0px'}}>
      <div className='md:w-[90%] w-[100%] mx-auto flex md:flex-row flex-col bg-slate-50 rounded-lg' style={{ fontFamily: 'Ubuntu, sans-serif' }}>
        <div className='md:w-auto w-[100vw] md:mx-2 mx-auto'>
          <h1 className=' lg:text-xl text-lg text-gray-600 my-2 font-semibold  mt-2'>{orders?.length !== 0 ? 'Order ID:' : ''} {orders.length !== 0 ? id : ''}</h1>
        ...
        <div className='md:w-auto w-[100vw] md:ml-5 ml-auto md:px-auto px-2'>
      ...
      <div className=' relative md:top-10 mx-4 sm:top-[1rem] vsm:top-[1rem] top-[1rem]'>
        ...
        <div className='grid lg:grid-cols-5 sm:grid-cols-4 grid-cols-2  mt-6  justify-center lg:w-auto '>
          {recentlyViewed !== undefined && (recentlyViewed.length > 10 ? recentlyViewed.slice(0,10) : recentlyViewed).reverse().map((viewedProduct) => {
            return (
                ...
                <div className='lg:h-[23rem] md:h-[21rem] h-[18rem]  md:w-56 w-[11rem] mx-auto rounded-md pb-4 group'>
                  <div className='lg:w-56 md:w-40 w-36 lg:h-44 h-36 mx-auto' onClick={() => dispatch(fetchCategoryProduct(viewedProduct.category))}>
                    <Link href={`details/${viewedProduct.id}`}>
                      <img src={viewedProduct.thumbnail} alt="" srcSet="" className='w-full h-full mx-auto cursor-pointer' />
                    </Link>
                  </div>
                  <div className='bg-green-600  flex py-1 px-1 my-2 w-16 align-middle text-white text-center rounded-sm ml-3'>{viewedProduct.rating}<i className="fa fa-star my-auto mx-1" />
                  </div>
                  <h1 className='font-bold lg:my-4 my-2 lg:text-lg text-sm ml-3'>{viewedProduct?.title.length > 20 ? `${viewedProduct?.title.slice(0, 18)}...` : viewedProduct.title}</h1>
                  <p className='text-gray-500 lg:ml-3 mx-1 md:text-lg text-[14px]'>${viewedProduct?.description.length > 40 ? `${viewedProduct?.description.slice(0, 40)}...` : viewedProduct.description}</p>
